...
namespace seann {
    ...
    __global__ void adaDeltaApplyD(Parameter* A, Tensor* V, float LEARNING_RATE, float EPSILON, float BETA){
        uint32 idx = threadIdx.x + blockDim.x * blockIdx.x;
        if(idx < A->a->dims.size){
            float gradVal = A->grad->elements[idx];
    ...
    __global__ void adaDeltaApply4D(Parameter* A, Tensor* V, float LEARNING_RATE, float EPSILON, float BETA){
        uint32 idx = (threadIdx.x + blockDim.x * blockIdx.x) * 4;
        float regisA[4] = {0};
        float regisG[4] = {0};
        float regisV[4] = {0};
    ...
    void AdaDelta::apply() {
        ...
        if(A->a->dims.size % 4 == 0){
            grid = topOff(A->a->dims.size, block * 4);
            adaDeltaApply4D<<<grid, block>>>(A,  V, LEARNING_RATE, EPSILON, BETA);
            assertCuda(__FILE__, __LINE__);
            return;
        }
        adaDeltaApplyD<<<grid, block>>>(A,  V, LEARNING_RATE, EPSILON, BETA);
        assertCuda(__FILE__, __LINE__);
    }
